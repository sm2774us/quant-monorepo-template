...
The original `systematic-monorepo.zip` skeleton wired up the correct
*shape* of a polyglot quant monorepo (Nx workspace → FastAPI app → nanobind
C++ extension → React dashboard) but the compute core was a stub: it
special-cased `window_sec == 2.0 and num_trades == 5` and fabricated a
`100.0 + i*0.5` VWAP curve instead of computing one. This rebuild replaces
every stub with a real, tested implementation while keeping the exact same
project shape, so the repo still works as a template for structuring
world-class quant monorepos.
...
4. **`release`** — on push to `main` after all three succeed: computes the
   next semantic version, generates a categorized changelog from merged
   PRs since the last tag via `mikepenz/release-changelog-builder-action`,
   prepends it to the source-controlled `CHANGELOG.md` and commits that
   change back to `main` (`[skip ci]` to avoid a trigger loop), then tags
   *that commit* — not a stale pre-changelog commit, which was a
   correctness bug in the naive version of this job. It builds a
   `git archive` source tarball, fails the build if `CHANGELOG.md` is
   somehow absent from it, generates a SHA-256 checksum, and publishes
   both as GitHub Release assets alongside the changelog as release notes.
   Changelog text is passed to shell steps exclusively via `env:` (never
   interpolated into `run:` strings), which closes a script-injection
   vector present in the original draft of this job: a PR titled with
   backticks or `$(...)` could otherwise execute arbitrary shell in the
   release runner. A `release-main` concurrency group and a pre-tag
   existence check prevent two concurrent pushes to `main` from racing to
   publish duplicate or conflicting tags.
